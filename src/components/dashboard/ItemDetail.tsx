...
export default function ItemDetail({ item, imageUrl }: { item: Item; imageUrl: string | null }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 rounded-3xl border border-white/60 bg-white/70 p-8 shadow-[0_10px_40px_-16px_rgba(76,29,149,0.25)] backdrop-blur-xl">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            <motion.span
              whileHover={{ rotate: -6, scale: 1.05 }}
              className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-100 to-fuchsia-100 text-2xl"
            >
              📦
            </motion.span>
            <div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          {facts.map((fact, i) => (
            <motion.div
              ...
              whileHover={{ y: -3 }}
              className="rounded-2xl bg-gradient-to-br from-violet-50/80 to-fuchsia-50/50 p-4"
            >
